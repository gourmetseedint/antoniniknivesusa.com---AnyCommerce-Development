...
//you may or may not need it.
        app.util.dump('BEGIN app.ext.myRIA.callbacks.init.onError');
        }
      },
  ...
  if(typeof myAppIsLoaded == 'function'){
    ...
    itemAddedToCart : {
      onSuccess : function(tagObj)  {
        app.util.dump('BEGIN app.ext.store_product.callbacks.itemAddedToCart.onSuccess');
        ...
        $('#atcMessaging_'+app.data[tagObj.datapointer].product1).append(app.util.formatMessage({'message':'Item Added','htmlid':htmlid,'uiIcon':'check','timeoutFunction':"$('#"+htmlid+"').slideUp(1000);"}));
        },
      onError : function(responseData,uuid) {
        ...
        $('#atcMessaging_'+app.data[responseData['_rtag'].datapointer].product1).append(app.util.getResponseErrors(responseData))
        }
      }, //itemAddedToCart
    ...
    showProd :  {
      onSuccess : function(tagObj)  {
        app.util.dump("BEGIN myRIA.callbacks.showProd");
//        app.util.dump(tagObj);
        app.renderFunctions.translateTemplate(app.data[tagObj.datapointer],tagObj.parentID);
//        app.util.dump(" -> resetting addthis.toolbox");
        },
      onError : function(responseData,uuid) {
...
//executed as part of init. shows checkout button if cart is populated.
    handleCheckoutButton : {
      onSuccess : function(tagObj)  {
        app.util.dump("BEGIN myRIA.callbacks.handleCheckoutButton.onSuccess ["+app.data.cartItemsList.cart['data.item_count']+"]");
        if(app.data.cartItemsList.cart['data.item_count'] >= 1)
...
//executed when the cart is changed, such as a zip entered or a country selected.
    cartUpdated : {
      onSuccess : function(tagObj)  {
        app.util.dump("BEGIN myRIA.callbacks.cartUpdated.onSuccess");
...
//requires templateID and targetID to be passed on the tag object.
    showProdList : {
      onSuccess : function(tagObj)  {
//        app.util.dump("BEGIN myRIA.callbacks.showProdList");
//        app.util.dump(app.data[tagObj.datapointer]);
        if(app.data[tagObj.datapointer]['@products'].length < 1)  {
          $('#'+tagObj.targetID).append(app.util.formatMessage('This list ('+listID+') appears to be empty.'));
          }
        else  {
          app.ext.store_prodlist.util.buildProductList({"withInventory":1,"withVariations":1,"templateID":tagObj.templateID,"parentID":tagObj.targetID,"csv":app.data[tagObj.datapointer]['@products']})
          app.model.dispatchThis();
          app.util.dump(" -> now is the time on sprockets when we carousel");
    ...
    handleElasticResults : {
      onSuccess : function(tagObj)  {
        app.util.dump("BEGIN myRIA.callbacks.handleElasticResults.onSuccess.");
        var L = app.data[tagObj.datapointer]['_count'];
        app.util.dump(" -> Number Results: "+L);
        $parent = $('#'+tagObj.parentID).empty().removeClass('loadingBG').show();
        if(L == 0)  {
          app.util.dump("WARNING! The query returned no results. No zero results message output so site aesthetic would be preserved.");
//          $parent.append("Your query returned zero results.");
    ...
    showResults :  {
      onSuccess : function(tagObj)  {
//        app.util.dump('BEGIN app.ext.myRIA.callbacks.showResults.onSuccess');
...
//        app.util.dump(' -> altSearchList emptied.');
...
//returns # of requests needed. so if 0 is returned, no need to dispatch.
          app.ext.store_prodlist.util.buildProductList({"templateID":"productListTemplate","withInventory":1,"withVariations":1,"parentID":"resultsProductListContainer","items_per_page":20,"csv":app.data[tagObj.datapointer]['@products']})
          }
...
//          app.util.dump(" -> more than 1 keyword was searched for.");
          $('#altSearchesList').addClass('loadingBG');
          app.ext.store_search.util.getAlternativeQueries(keywords,{"callback":"updateSearchNav","extension":"myRIA"});
          }
        else  {
          $('#altSearchesContainer').hide();
          }
        app.ext.myRIA.util.showRecentSearches();
...
*/
    wiki : {
      ...
      ":product" : function(suffix,phrase){
        return "<a href='#' onClick='app.ext.myRIA.util.showContent(\"product\",\""+suffix+"\"); return false;'>"+phrase+"<\/a>"
        },
...
////////////////////////////////////   RENDERFORMATS    \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    ...
    renderFormats : {
      legacyURLToRIA : function($tag,data)  {
        app.util.dump("BEGIN control.renderFormats.legacyURLToRIA");
        app.util.dump(" -> data.value: "+data.value);
        if(data.value == '#') {
          $tag.removeClass('pointer');
          }
        else  {
          pageType = app.ext.myRIA.util.whatPageTypeAmI(data.value);
          pageInfo = app.ext.myRIA.util.giveMeRelevantInfoToPage(data.value);
          app.util.dump(" -> pageType: "+pageType);
          app.util.dump(" -> pageInfo: "+pageInfo);
          $tag.addClass('pointer').click(function(){
            app.ext.myRIA.util.showContent(pageType,pageInfo,true);
            });
          }
        },
...
app.util.dump("BEGIN store_product.renderFunctions.addToCartButton");
// app.util.dump(data);
$tag.addClass('chooseOptionsButton').bind('click.myATCEvent',function(event){
  event.preventDefault();
  app.ext.myRIA.util.showProd($(this).parent().attr('data-pid'));
  }).text('Choose Options')
...
//use original pdata vars for display of price/msrp. use parseInts for savings computation only.
      ...
      if(price > 0) {
        ...
        o += app.util.formatMoney(pData['zoovy:base_price'],'$',2,true)
        o += "<\/span><\/div>";
//only show the msrp if it is greater than the price.
        if(msrp > price)  {
          ...
          o += app.util.formatMoney(pData['zoovy:prod_msrp'],'$',2,true)
          o += "<\/span><\/div>";
//don't bother with savings of less than a buck.
          if(msrp-price > 1)  {
            ...
            o += app.util.formatMoney(msrp-price,'$',2,true)
            o += "<\/span><\/div>";
            }
          }
        }
...
var tagObj = {"callback":"handleElasticResults","extension":"myRIA","parentID":P.parentID,"datapointer":"elasticsearch|"+app.util.unixNow()};
...
app.model.dispatchThis();
          ...
          app.util.dump("WARNING! either parentID ["+P.parentID+"] or query [typeof: "+typeof P.query+"] were not set in showCarousel. both are required.");
          }
        }
...
//        app.ext.store_product.util.prodDataInModal({'pid':pid,'templateID':'productTemplate',});
...
// a two step process here to open the dialog allows for the same dialog to be re-opened.
      ...
      showCart : function() {
//        app.util.dump("BEGIN myRIA.util.showCart");
        app.ext.store_cart.util.showCartInModal('cartTemplate');
        },
  
      handleAddToCart : function(formID)  {

app.util.dump("BEGIN store_product.calls.cartItemsAdd.init");
$('.checkoutNowButton').show();
$('#'+formID+' .atcButton').addClass('disabled').attr('disabled','disabled');
if(!formID) {
  //app error
  app.util.dump("APP ERROR - no form id handed to myRIA.util.handleAddToCart");
  }
else  {
