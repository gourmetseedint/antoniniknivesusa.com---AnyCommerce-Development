...
//adds submit functionality to search form. keeps dom clean to do it here.
        ...
        app.ext.myRIA.vars.mcSetInterval = setInterval(function(){
          app.ext.myRIA.u.handleMinicartUpdate({'datapointer':'cartDetail'});
        },4000); //make sure minicart stays up to date.
        ...
        if(typeof app.u.appInitComplete == 'function'){
          app.u.appInitComplete(page);
        } //gets run after app has been init
        }
      }, //startMyProgram
...
// product lists are being handled through 'buildProductList'.
        ...
        L = tagObj.searchArray.length;
        var $parent;
        for(var j = 0; j < L; j += 1) {
          $parent = $('#'+tagObj.searchArray[j].split('|')[1]);
          app.ext.myRIA.renderFormats.productSearch($parent,{value:app.data[tagObj.searchArray[j]]});
          }
        tagObj.state = 'onCompletes'; //needed for handleTemplateFunctions.
        app.ext.myRIA.u.handleTemplateFunctions(tagObj);
...
//          app.u.dump(prods);
          app.ext.store_prodlist.u.buildProductList({"templateID":"productListTemplateBuyerList","withInventory":1,"withVariations":1,"parentID":tagObj.parentID,"csv":prods,"withReviews":1});
          app.model.dispatchThis();
          }
        }
      }, //showList
...
// if we redirect to ssl for checkout, it's a new url and a pushstate isn't needed, so a param is added to the url.
              ...
              SSLlocation = app.vars.secureURL+"?sessionId="+app.sessionId+"#checkout?show=checkout";
              _gaq.push(['_link', SSLlocation]); //for cross domain tracking.
              document.location = SSLlocation;
              }
            else  {
...
//best practice would be to NOT call this function directly. call showContent.
      ...
      showPage : function(P)  {
        ...
        if(!catSafeID)  {
          ...
          P.state = 'onInits';
          app.ext.myRIA.u.handleTemplateFunctions(P);
          parentID = P.parentID || P.templateID+'_'+app.u.makeSafeHTMLId(catSafeID);
...
var myAttributes = []; // used to hold all the 'page' attributes that will be needed. passed into appPageGet request.
...
tagObj.searchArray = []; //an array of search datapointers. added to _tag so they can be translated in showPageContent
tagObj.extension = 'myRIA';
tagObj.lists = []; // all the list id's needed.
...
//        app.u.dump("BEGIN myRIA.u.handleMinicartUPdate"); app.u.dump(tagObj);
        ...
        if(app.data[tagObj.datapointer] && app.data[tagObj.datapointer].sum)  {
          r = true;
          itemCount = app.u.isSet(app.data[tagObj.datapointer].sum.items_count) || 0;
          subtotal = app.data[tagObj.datapointer].sum.items_total;
          total = app.data[tagObj.datapointer].sum.order_total;
          }
        else  {
